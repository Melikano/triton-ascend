...
| **Compilation control**| TRITON_ASCEND_COMPILE_FLOW | **npuir** | Selects the Triton-Ascend kernel compilation flow. | **npuir**: Use the native NPU-IR flow.<br>**ptoas**: Use the experimental NPU-IR-to-PTOAS flow (`ttir→ttadapter→mlirbc→bcmlir→ptovmi→npubin`). A5 vector kernels only. | |
...
| **Compilation control**| TRITON_PTOAS_PATH | Not set | Specifies the PTOAS compiler executable used by `TRITON_ASCEND_COMPILE_FLOW=ptoas`. | Full path to `ptoas`. If unset, `ptoas` is searched on `PATH`. | |
| **Compilation control**| TRITON_OBJCOPY_PATH | Not set | Specifies the `objcopy` executable used to extract `__aicore_rel_binary` from the PTOAS fat object. | Full path to `objcopy`. If unset, `objcopy` is searched on `PATH`. | |
| **Compilation control**| TRITON_AICORE_LD_PATH | Not set | Optional override for the AICore `ld.lld` executable used to link the extracted PTOAS relocatable object into `npubin`. | Normally leave unset. The compiler derives this from `$ASCEND_HOME_PATH/tools/bisheng_compiler/bin/ld.lld`; if that path is missing, it falls back to `ld.lld` on `PATH`. | |
...
| **Compilation flow** | `compile_flow` | `"npuir"` (default), `"ptoas"` | Selects the per-kernel backend flow. Explicit launch/config values override `TRITON_ASCEND_COMPILE_FLOW`. `"ptoas"` is experimental and currently limited to A5 vector kernels without sync-block-lock resource requirements. | `triton.Config` or launch meta-parameter |
